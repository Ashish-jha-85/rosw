...
# -----------------------------------------------------------------------------------------------------------------
            ...
            target[2] += self.approach_offset_z 
...
# -----------------------------------------------------------------------------------------------------------------------
            ...
            if not self.slowPhase:
                if self.move_to_tcp_target(hover_target, self.approach_tol):
                    ...
                    self.get_logger().info("Hover reached. Descending now...")
                    self.slowPhase = True # Enable next step
            
            # If slowPhase is True, we are DESCENDING
            else:
                # We use 'if' here to wait until move_to_tcp_target returns TRUE
                if self.move_to_tcp_target(actually_target, self.approach_tol, slow=True):
...
# -------------------------------------------------------------------------------------------------
            ...
            fruit_record = self.badFruitTable[self.current_fruit_index]
            lift_target = fruit_record['pos'].copy()  # Use key name
            lift_target[2] += 0.25
